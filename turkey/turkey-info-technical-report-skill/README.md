...
抓取 Info Yatırım **Teknik Bülten**（技术分析公告），生成**表格为主**的中文技术分析报告。

## 开箱即用

| 必需 | 可选（推荐） |
|------|------|
| Python 3.9+ | Turkey-investment 本地缓存（表格更完整） |
| `pip install -r requirements.txt` | Cron / 推送（自行配置） |
| LLM API Key（`max_tokens ≥ 12000`） | |
| 复制本目录 + `config.json` | |
...
源站发布：TR ~12:00 / 北京 ~17:00

| 北京时间 | 动作 |
|----------|------|
| **17:20** | 首发 |
| 17:30–18:10 | 每 10 分钟重试（共 6 次） |
| **20:00** | 兜底 |
| 周末/土节假日 | 跳过 |
