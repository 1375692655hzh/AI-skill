...
| 文件 | 说明 |
|------|------|
| `output/{日期}_close_report_zh.md` | 完整收评 |
| `output/{日期}_close_report_brief_zh.md` | 结构化简报（汉字+中文标点 200–600 字，个股每只一行） |
...
→ 等到北京 00:05～00:15 再跑（或 23:55 失败后在 00:15 重试）。文章 CMS 戳≈23:30 不等于 `/borsa`/列表已可见。
...
1. **推荐主跑 `00:05～00:15`**（TR ≈ 19:05～19:15）—— 覆盖「文章已发但 `/borsa` 相关区尚未挂上」的滞后。
2. **备选 `23:55` + `00:15` 失败重试 1 次**—— 重试条件：`list_absent` / BHT 无文 / `gen_failed`；当日文件已成功则跳过。

Skill 在列表未上榜时会快速失败（不 ID 海扫），第二轮靠 runner 调度。
