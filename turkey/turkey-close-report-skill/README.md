...
- **BloombergHT `/borsa/` 实时行情（最快行情源）**：XU100、涨跌幅居前、跌幅居前、成交额居前个股
- **BloombergHT 详细收盘（核心数据源）**：日内高低点、板块、汇率、黄金、原油
...
注意：收盘数据（指数收盘价、涨跌幅、成交量、USD/TRY、EUR/TRY、黄金、原油）**必须以 BloombergHT 为准**。其中 XU100 与个股快照优先取 `/borsa/`，其余收盘字段取收盘综述文章。
`【大盘概况】` / `【关键个股异动】` / `【行业板块表现】` / `【汇市与大宗商品】` 四节 **100% 只用 BHT、不加分析**；成交额用「亿里拉」；汇市大宗不写钟点。  
Paraborsa / Info 仅用于 `【核心信号与逻辑】` / `【后市策略参考】`。
...
| 方案 | 主跑 | 重试 | 说明 |
|------|------|------|------|
| **推荐** | **00:05～00:15** | 一般不需要 | 比 `/borsa` 相关区可见（约 23:50+）再留 15～25 分钟 |
| 备选 | 23:55 | **00:15 一次** | 仅 `list_absent` / 无文 / `gen_failed` 时重试 |

文章 CMS 时间戳常为 TR 18:30 / 北京 23:30，**不等于**列表或 `/borsa` 已可抓到。
...
| 源 | 时间 | 角色 | 内容 |
|---|------|------|------|
| BloombergHT `/borsa/` 实时行情 | 交易时段/收盘后 | **核心** | XU100、涨跌幅居前、跌幅居前、成交额居前个股 |
| BloombergHT 详细收盘 | TR ~18:30 | **核心** | 日内高低点、板块、汇率、黄金、原油 |
...
A: 可能是还没收盘，或文章未挂上列表/`/borsa`。建议主跑放到北京 **00:05～00:15**；若 23:55 已跑过且失败，在 **00:15** 再试一次。
