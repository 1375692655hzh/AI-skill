#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Main entry: generate Turkey Info technical bulletin Chinese report."""
...
from fetch_info_technical import fetch_info_technical
...
def generate(config_path: Path, force_date: str | None = None, no_llm: bool = False) -> Path | None:
    ...
    skill_dir, workdir, output_dir, cache_dir, template_path = resolve_paths(
        config_path,
        config,
        default_template="templates/info_technical_report_template.txt",
        default_cache=".cache/turkey-info-technical-report",
    )
    os.chdir(workdir)
    ...
    source_cfg = config.get("sources", {}).get("info_technical", {})
    ...
    if not bulletin.get("ok") or not content:
        ...
        fallback = bulletin.get("fallback_url", "https://infoyatirim.com/arastirma/teknik-bulten")
        print(f"Technical bulletin not available for {target_date} (reason={reason}).")
    ...
    prompt_file = cache_dir / f"technical_prompt_{target_date.isoformat()}.txt"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate Turkey Info technical bulletin report")
